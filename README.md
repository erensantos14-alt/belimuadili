...
5. **Project Settings → API Keys** sayfasından şu ikisini kopyala:
...
1. **Arkadaş takibi ve akış** — `follows` tablosu şemada hazır, arayüzü yok.
   Uygulamanın sosyal olmasını sağlayan şey bu; en kritik eksik.
2. **Profil sayfası** — kullanıcı adıyla herkese açık liste (`/@kullaniciadi`).
3. **Fotoğraf** — `entries.photo_path` alanı hazır; Supabase Storage bağlanacak.
4. **Mekan sayfası** — kimler gitmiş, arkadaş ortalaması, Ara / Instagram /
   Yol tarifi butonları.
5. **Kategoriler** — `places.category` hazır; şimdilik sadece `kahve` dolu.
